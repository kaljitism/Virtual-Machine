...
#define MAKE_INSTRUCTION_EQUALITY() {.type = INSRUCTION_EQUALITY}
...
int stringView_toInteger(StringView stringView) {
  ...
  for ( size_t i = 0;
        i < stringView.count && isdigit(stringView.data[i]); ++i ) {
...
Instruction vm_translate_line(StringView line) {
  ...
  if ( stringView_equal(
      instructionName,
      cstr_as_stringView("push"))) {
    line = stringView_trim_left(line);
    int operand = stringView_toInteger(stringView_trim_right(line));
    return (Instruction) {.type = INSTRUCTION_PUSH, .operand = operand};
  }
  
  else if ( stringView_equal(
      instructionName,
      cstr_as_stringView("nope"))) {
    line = stringView_trim_left(line);
    return (Instruction) {.type = INSTRUCTION_NOPE};
  }
  
  else if ( stringView_equal(
      instructionName,
      cstr_as_stringView("duplicate"))) {
    line = stringView_trim_left(line);
    int operand = stringView_toInteger(stringView_trim_right(line));
    return (Instruction) {.type = INSTRUCTION_DUPLICATE, .operand = operand};
      ...
      cstr_as_stringView
          ("printDebug"))) {
    line = stringView_trim_left(line);
    return (Instruction) {.type = INSTRUCTION_PRINT_DEBUG};
  }
  
  else if ( stringView_equal(
      instructionName,
      cstr_as_stringView("plus"))) {
    line = stringView_trim_left(line);
    return (Instruction) {.type = INSTRUCTION_PLUS};
  }
      ...
      instructionName,
      cstr_as_stringView("minus"))) {
    line = stringView_trim_left(line);
    return (Instruction) {.type = INSTRUCTION_MINUS};
  }
  
  else if ( stringView_equal(
      instructionName,
      cstr_as_stringView("multiply"))) {
    line = stringView_trim_left(line);
    return (Instruction) {.type = INSTRUCTION_MULTIPLY};
  }
  
  else if ( stringView_equal(
      instructionName,
      cstr_as_stringView("divide"))) {
    line = stringView_trim_left(line);
    return (Instruction) {.type = INSTRUCTION_DIVIDE};
  }
  
  else if ( stringView_equal(
      instructionName,
      cstr_as_stringView("equal"))) {
    line = stringView_trim_left(line);
    return (Instruction) {.type = INSTRUCTION_EQUALITY};
  }
    ...
    line = stringView_trim_left(line);
    int operand = stringView_toInteger(stringView_trim_right(line));
    return (Instruction) {.type = INSTRUCTION_JUMP_IF, .operand = operand};
  }
  
  else if ( stringView_equal(
      instructionName,
      cstr_as_stringView("halt"))) {
    line = stringView_trim_left(line);
...
size_t vm_translate_source(
  ...
  while ( sourceCode.count > 0 ) {
    ...
    StringView line = stringView_trim(stringView_chop_by_delimeter(
        &sourceCode,
        '\n'));
    if ( line.count > 0 ) {
...
StringView slurp_file(const char *filePath) {
  ...
  char *buffer = malloc(currentPosition);
  if ( buffer == NULL) {
    fprintf(stderr,
            "Could not allocate memory to the file: %s\n",
            strerror(errno));
    exit(1);
  }
  ...
  size_t count = fread(buffer, 1, currentPosition, file);
  if ( ferror(file)) {
    fprintf(stderr,
